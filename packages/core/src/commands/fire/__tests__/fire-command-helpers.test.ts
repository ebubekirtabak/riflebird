...
import {
  ...
  shouldExcludeFile,
  getPatternsFromInput,
} from '../fire-command-helpers';
...
describe('fire-command-helpers', () => {
  describe('resolveTestTypes', () => {
    ...
    it('should return default ["unit", "document"] when `all` is false/undefined and `testTypes` is empty', () => {
      expect(resolveTestTypes(false, [])).toEqual(['unit', 'document']);
      expect(resolveTestTypes(undefined, [])).toEqual(['unit', 'document']);
    });
  ...
  describe('getPatternsFromInput', () => {
    it('should return specific file path when testPath is a single file', () => {
      const input = { testPath: 'src/components/Button.tsx' };
      const patterns = getPatternsFromInput(input);
      expect(patterns).toEqual(['src/components/Button.tsx']);
    });

    it('should return pattern when testPath is a glob pattern', () => {
      const input = { testPath: 'src/**/*.test.ts' };
      const patterns = getPatternsFromInput(input);
      expect(patterns).toEqual(['src/**/*.test.ts']);
    });

    it('should return default pattern when no input provided', () => {
      const input = {};
      const patterns = getPatternsFromInput(input);
      expect(patterns).toEqual(['src/**/*.{ts,tsx,js,jsx,vue}']);
    });

    it('should return scope patterns when scope is provided', () => {
      const input = { scope: 'component' as TestScope };
      const patterns = getPatternsFromInput(input);
      expect(patterns).toContain('**/src/components/**/*.{tsx,jsx,vue}');
    });
  });
